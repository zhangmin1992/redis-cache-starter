...
public class CheckUtils {
    ...
    public static  boolean isBlank(String str) {
    	boolean result = (str.equals("") || str == null);
    	return result;
    }
    ...
    /**
     * 验证对象集合是否为空 (空字符串，空数组，空的Collection或Map(只有空格的字符串也认为是空串))
     *
     * @param objs     被验证的对象
     * @param messages 异常信息
     */
    public static void notEmpty(Object[] objs, String... messages) {
        if (objs.length != messages.length) {
            throw new IllegalArgumentException(
                    "objs's length is not equlas message's length");
        }
        for (int i = 0; i < objs.length; i++) {
            notEmpty(objs[i], messages[i]);
        }
    }
